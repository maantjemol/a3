...
from dynprog_main import DroneExtinguisher
...
class TestDroneExtinguisher(unittest.TestCase):
    ...
    def test_compute_sequence_idle_time_in_liters(self):
        ...
        i = 0
        for j in range(3):
            self.assertEqual(de.compute_sequence_idle_time_in_liters(
                i, j), idle_time_in_liters[j])
    ...
    def test_compute_idle_cost(self):
        ...
        i = 0
        for j in range(3):
            self.assertEqual(de.compute_idle_cost(
                i, j, idle_time_in_liters[j]), idle_costs[j])
    ...
    def test_dynamic_programming_simple(self):
        ...
        bags = [6, 3, 8, 9]
        bag_locations = [(0, 0)
                         for _ in range(len(bags))]  # no travel distance
